...
def MADD(h, X_test=None, pred_proba=None, sf=None, pred_proba_sf0=None, pred_proba_sf1=None, min_nb_points=50, model=""):
    """Computes the MADD.
    ...
    if h == "auto":
        ...
        h_opt = (interval["h end"]-interval["h start"])/2.0
        D_G0 = normalized_density_vector(pred_proba_sf0, h_opt)
        D_G1 = normalized_density_vector(pred_proba_sf1, h_opt)
        #madd_plot(h_opt, pred_proba_sf0, pred_proba_sf1, sf, "MADD_"+sf, model)

        with open("maddstats.csv", 'a+') as f:
            f.write(",".join([model, sf, str(h_opt), str(np.mean(pred_proba_sf0)), str(np.mean(pred_proba_sf1)), str(np.max(pred_proba_sf0)), str(np.max(pred_proba_sf1))] )+"\n" )
...
def madd_plot(h, pred_proba_sf0, pred_proba_sf1, legend_groups, title, model="", figsize=(12, 4)):
    ...
    plt.savefig("img/"+legend_groups+model+str(np.random.random())+".png");